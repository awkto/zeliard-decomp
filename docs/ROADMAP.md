...
5. select.bin (status screen + potion effects) — DONE (#26): `src/select.c`, `docs/TOWN.md` §12 (both entry vectors, the four windows, the three rows, the eight potion effects, the INVENTORY window); ported in `port/status.c` and verified pixel-exact against `docs/screenshots/menu.png`. Remaining: mole.bin, opdemo/enddemo/rokademo (cutscenes last).
...
3. Milestone gates: ~~(a) walk around cavern 1 with correct collision~~ DONE (#23); ~~(b) kill one enemy with correct damage tables~~ DONE (#24); (c) full cavern 1 + town loop; (d) all 9 caverns; (e) audio.
