...
5. select.bin (status screen + potion effects) — DONE (#26): `src/select.c`, `docs/TOWN.md` §12 (both entry vectors, the four windows, the three rows, the eight potion effects, the INVENTORY window); ported in `port/status.c` and verified pixel-exact against `docs/screenshots/menu.png`. mole.bin also decoded (Sprint 17): it is the **boot-time screen painter**, not a cutscene — GAME.BIN far-calls it once to draw the stone frames, the strip above the playfield and the grey HUD panel; RLE + mode-4 blitter reimplemented in `port/`.  Remaining: opdemo/enddemo/rokademo (cutscenes last).
